...
class AACEncodeProcessor(strategy: IAudioStrategy? = null) : AbstractProcessor(false) {
    ...
    /**
     * Record mp3 start - DISABLED karena masalah native library (LameMp3)
     */
    fun recordMp3Start(audioPath: String?, callBack: ICaptureCallBack) {
        mMainHandler.post {
            callBack.onError("MP3 recording temporarily disabled (native library issue)")
        }
        Logger.w(TAG, "MP3 recording disabled due to native compatibility")
    }
